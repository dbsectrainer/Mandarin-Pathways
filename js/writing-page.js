...
const writingData = {
    character: {
        ...
        "Complete Radicals - Group 1": {
            ...
            description:
                "Practice common radicals (1-30 of 214 Kangxi radicals)",
        ...
        "Complete Radicals - Group 3": {
            ...
            description:
                "Practice common radicals (61-90 of 214 Kangxi radicals)",
        ...
        "Theme - Family": {
            ...
            descriptionPinyin:
                "Liànxí yǔ jiātíng chéngyuán hé guānxì xiāngguān de Hànzì",
            hasAudio: true,
        },
        "Theme - Food": {
            title: "Theme - Food",
        ...
        "Theme - Travel": {
            ...
            description:
                "Practice characters related to travel and transportation",
            descriptionZh: "练习与旅行和交通相关的汉字",
            descriptionPinyin:
                "Liànxí yǔ lǚxíng hé jiāotōng xiāngguān de Hànzì",
            hasAudio: true,
        },
    },
    sentence: {
        ...
        Advanced: {
            ...
            descriptionPinyin:
                "Yòng shìdàng de cíyǔ huò duǎnyǔ wánchéng fùzá jùzi",
            hasAudio: true,
        },
    },
    translation: {
        Beginner: {
            ...
            description:
                "Translate simple sentences between English and Chinese",
            descriptionZh: "翻译英文和中文之间的简单句子",
            descriptionPinyin:
                "Fānyì Yīngwén hé Zhōngwén zhījiān de jiǎndān jùzi",
            hasAudio: true,
        },
        Intermediate: {
            ...
            descriptionPinyin:
                "Fānyì Yīngwén hé Zhōngwén zhījiān de gèng fùzá jùzi",
            hasAudio: true,
        },
        Advanced: {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (
        activityType &&
        level &&
        writingData[activityType] &&
        writingData[activityType][level]
    ) {
    ...
    document.getElementById("check-btn").addEventListener("click", () => {
        ...
        exercises.forEach((exercise) => {
            const input = exercise.querySelector(
                ".writing-input, .answer-input",
            );
    ...
    const completedWritings = JSON.parse(
...
function setupWritingSectionHeader(activityInfo, lang) {
    ...
    if (lang === "pinyin") {
        ...
        titleCue.textContent = activityInfo.titlePinyin || activityInfo.title;
    ...
    const parts = activityInfo.title
        .split(" / ")
        .map((s) => s.trim())
        .filter(Boolean);
...
function loadWritingContent(activityType, level, lang) {
    ...
    if (activityInfo.hasAudio) {
        ...
        audio.src = "";
        audioFallback.innerHTML =
            '<p class="note"><i class="fas fa-info-circle"></i> Audio not available for this activity.</p>';
        audioFallback.style.display = "block";
    }

    const txtUrl = `writing_files/${activityType}_${levelSlug}_${lang}.txt`;
    const timingUrl = activityInfo.hasAudio
        ? `timing/writing/${activityType}_${levelSlug}_${audioLang}.json`
        : "";
...
function formatCharacterPractice(lines, container) {
    ...
    zhInstruction.innerHTML =
        '<strong>练习方法：</strong>使用鼠标或触摸屏在绘图框中绘制每个汉字。您可以使用"清除"、"撤销"和"提示"按钮来辅助练习。';

    const pinyinInstruction = document.createElement("span");
    pinyinInstruction.className = "pinyin";
    pinyinInstruction.innerHTML =
        '<strong>Liànxí fāngfǎ:</strong> Shǐyòng shǔbiāo huò chùmō píng zài huìtú kuàng zhōng huìzhì měi gè hànzì. Nín kěyǐ shǐyòng "qīngchú", "chèxiāo" hé "tíshì" ànniǔ lái fǔzhù liànxí.';

    const enInstruction = document.createElement("span");
    enInstruction.className = "en";
    enInstruction.innerHTML =
        "<strong>How to practice:</strong> Use your mouse or touch screen to draw each character in the drawing boxes. You can use the Clear, Undo, and Hint buttons to help with your practice.";
