...
async def generate_audio(category, voice="zh-CN-XiaoxiaoNeural"):
    """Generate audio file for a specific category using edge-tts"""
    print(f"\nGenerating {category} audio file...")
    ...
    # Generate audio
    await communicate.save(f"audio_files/supplementary/{category}_zh.mp3")
    
    elapsed = time.time() - start_time
    print(f"✓ Saved to audio_files/supplementary/{category}_zh.mp3 ({elapsed:.2f}s)")

async def main():
    parser = argparse.ArgumentParser(description="Generate supplementary Mandarin learning files")
    ...
    for category in categories_to_process:
        ...
        if not args.text_only:
            await generate_audio(category, args.voice)
    ...
    print("  - Generate with different voice: python mandarin_phrases_supplementary.py --voice zh-CN-YunxiNeural")
    print("\nAvailable voices:")
